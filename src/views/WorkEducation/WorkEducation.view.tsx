...
const LeftColumn = tw.div`flex flex-col justify-center text-left pt-20 pb-6 md:(flex-1 text-left justify-center max-w-[600px]) z-10`;
const RightColumn = tw.div`flex-[2 2] flex flex-col justify-start text-left md:(text-left justify-center px-20) z-10`;
...
const IconContainer = tw.div`inline-flex mt-1.5 w-8 flex-[0 0 32px] text-textWhite`;
const Line = styled.div<{ last?: boolean }>(({ last }) => [
  tw`ml-4 border-0 border-l-2 border-solid text-textWhite`,
  last && tw`border-l-0`,
]);
const ExperienceList = tw.ul`max-w-[600px]`;
...
const ListDescription = styled.div(({ expanded }: { expanded: boolean }) => [
  tw`ml-8 font-extralight text-textWhite text-justify text-sm mb-2 md:(text-base mb-6 [line-height: 1.5]) translate-x-full transition-all opacity-0 [transition-duration: 500ms] [transition-delay: 250ms]`,
